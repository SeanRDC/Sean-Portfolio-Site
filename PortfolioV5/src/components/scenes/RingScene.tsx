...
// ── SCENE CONFIGURATION (Resized to fit 18 items cleanly) ───────────────────
const IMG_W = 260;
const IMG_H = 180;

const SLICES = 10;
const SLICE_W = IMG_W / SLICES;
const CYL_R = 400; // The curve of the individual image
const TOTAL_BEND_DEG = (IMG_W / CYL_R) * (180 / Math.PI);
const STEP_DEG = TOTAL_BEND_DEG / SLICES;

// ── EXACT REFERENCE MATH CONFIGURATION ──────────────────────────────────────
const ORBIT_RX = 480; // rx: Radius X
const ORBIT_RZ = 550; // rz: Radius Z
const TILT_Y = 140; // tiltY: The Y-axis wave
const OFF_X = 1200; // offX: Entry/Exit distance
const ENTRY_ANGLE = Math.PI / 2;

// Stagger determines spacing. 0.08 spaces 18 items beautifully across the 360 loop.
const STAGGER = 0.08;
const TOTAL_RANGE = 1 + STAGGER * (N - 1);

// ── THE EXACT REFERENCE MATH (getPos) ───────────────────────────────────────
function getPos(t: number) {
  ...
  if (t <= 0.88) {
    ...
    return {
      x: x,
      y: (z / ORBIT_RZ) * TILT_Y, // The exact "wave" tilt from the reference
      z: z,
      rotY: ry,
    };
  }
...
export default function RingScene() {
  ...
  return (
    // 800vh provides a long, smooth scrolling experience for 18 items
    <section id="ring" ref={sectionRef} className="relative h-[800vh] bg-paper">
      <div className="sticky top-0 flex h-screen w-full items-center justify-center overflow-hidden border-b border-line">
        <div className="pointer-events-none absolute left-6 top-24 z-50 font-mono-x text-[10px] uppercase leading-relaxed text-ink-dim md:left-10">
          ...
          <div className="text-ink-faint">Gallery - 360 Sequence</div>
        </div>
            ...
            {TILES.map((t, i) => {
              ...
              return (
                ...
                >
                  ...
                  {/* Label - Only fully visible when passing the front */}
                  <div
        ...
        {/* HUD Progress Bar */}
        <div className="pointer-events-none absolute inset-x-6 bottom-8 z-50 flex items-center gap-4 md:inset-x-10">
        ...
        {/* ── NEW: CONTEXTUAL REDIRECT BUTTON ── */}
        <div
          className={`absolute bottom-20 right-6 z-[100] md:bottom-8 md:right-10 transition-all duration-700 ease-out ${
            progress > 0.02 && progress < 0.98
              ? "opacity-100 translate-y-0 pointer-events-auto"
              : "opacity-0 translate-y-8 pointer-events-none"
          }`}
        >
          {/* Change href to your actual route */}
          <a
          ...
          >
            <div className="relative flex h-full items-center justify-between px-5 font-mono-x text-[10px] font-bold uppercase tracking-[0.2em] text-paper">
              ...
              {/* Spinning Cross Icon from reference */}
